...
foreach($pdo->query('SELECT column_name FROM information_schema.columns 
    WHERE table_schema = "surveyx_questionnaire" AND table_name = "brondell_q";') as $row) {
    $col_name[] = $row[0];
}

$filename = "brondell-" . date("Y-m-d H-i-s") . ".csv";
...
foreach($pdo->query('select * from brondell_q;') as $row) {
    unset($result_arr);
